...
import { QR_MODULE_COUNT, buildQrModules } from "@/features/returns/presentation/qrCode";

export type PaymentSlipLogo = {
  width: number;
  height: number;
  rgb: Uint8Array;
};

export type PaymentSlipOptions = {
  storeName: string;
  /** Ignored for display — PDF uses WinAnsi and always prints `DA`. */
  currency?: string;
  logo?: PaymentSlipLogo | null;
};

/** WinAnsi PDF fonts cannot render Arabic/other non-Latin currency glyphs. */
export const PAYMENT_SLIP_CURRENCY = "DZD";
...
export const getPaymentSlipFileName = (code: string) =>
  `payment-acknowledgement-${code}.pdf`;
...
class SimplePdf {
  ...
  addImage = (image: PaymentSlipLogo) => {
...
export const buildPaymentSlipPdfBytes = (
  item: PaymentEntity,
  options: PaymentSlipOptions
): Uint8Array => {
  const pdf = new SimplePdf();
  const tracking = item.code;
  const lines = item.orders ?? [];
  const date = formatSlipDate(item.created_at);
  const storeName = options.storeName || "-";
  const currency = PAYMENT_SLIP_CURRENCY;
  const formattedAmount = item.amount.toLocaleString("en-US", {
    maximumFractionDigits: 0,
  });
  const amountLabel = `${formattedAmount} ${currency}`;
  ...
  pdf.text("PAYMENT ACKNOWLEDGEMENT", PAGE_WIDTH / 2, headerBottom + 28, {
    size: 9,
    bold: true,
    align: "center",
    color: MUTED,
  ...
  const receiptPrefix = "acknowledge receipt of the sum of";
  pdf.text(receiptPrefix, MARGIN, declarationStart + 26, { size: 12 });
  pdf.text(`${amountLabel}.`, MARGIN + estimateWidth(receiptPrefix, 12) + 6, declarationStart + 26, {
    size: 12,
    bold: true,
    color: PRIMARY,
  });
  ...
  const amountCol = PAGE_WIDTH - MARGIN - 12;
  ...
  pdf.text("Qty", qtyCol, headerBaseline, { size: 8, bold: true, color: MUTED });
  pdf.text("Amount", amountCol, headerBaseline, {
    size: 8,
    bold: true,
    color: MUTED,
    align: "right",
  ...
  if (lines.length === 0) {
    pdf.text("No orders", PAGE_WIDTH / 2, tableTop + headerH + 15, {
      size: 10,
      align: "center",
      color: MUTED,
    });
  ...
  lines.forEach((line, index) => {
    ...
    pdf.text(fitText(line.product || `Order #${line.order_id}`, productWidth, 10), productCol, y, {
      size: 10,
    });
    pdf.text(String(line.product_qty ?? 0), qtyCol, y, { size: 10 });
    pdf.text(
      `${line.amount.toLocaleString("en-US", { maximumFractionDigits: 0 })} ${currency}`,
      amountCol,
      y,
  ...
  pdf.text("Payment acknowledgement", MARGIN, PAGE_HEIGHT - 18, {
    size: 8,
    color: MUTED,
  });
...
const loadLogo = async (): Promise<PaymentSlipLogo | null> => {
...
export const downloadPaymentSlip = async (
  item: PaymentEntity,
  options: PaymentSlipOptions
): Promise<boolean> => {
  ...
  try {
    ...
    const bytes = buildPaymentSlipPdfBytes(item, {
      storeName: options.storeName,
      currency: options.currency,
      logo,
    });
    triggerDownload(bytes, getPaymentSlipFileName(item.code));
    return true;
  } catch {
    return false;
  }
